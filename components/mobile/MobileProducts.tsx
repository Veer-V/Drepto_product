...
const MobileProducts: React.FC = () => {
    ...
    const [cartTotal, setCartTotal] = useState<{ total: number, message: string } | null>(null);
    ...
    const handleCheckout = async () => {
        ...
        try {
            ...
            if (data.total === 0 || window.confirm(`Place order for ₹${data.total}?`)) {
    ...
    return (
        <div className="min-h-screen bg-gray-50 pb-20">
            ...
            <div className="p-4 grid grid-cols-2 gap-4">
                {products.length === 0 && <p className="col-span-2 text-center text-gray-500">No products available.</p>}
                {products.map((prod) => (
                    <div key={prod.id} className="bg-white p-4 rounded-xl shadow-sm flex flex-col items-center text-center">
                        <img
                            src={prod.images && prod.images.length > 0 ? prod.images[0] : `https://placehold.co/200x200?text=${prod.name}`}
                            alt={prod.name}
