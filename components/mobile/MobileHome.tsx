    ...
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      <div className="bg-white p-4 rounded-b-3xl shadow-sm mb-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={() => navigate('/auth')} className="h-10 w-10 bg-gray-100 rounded-full flex items-center justify-center border border-gray-200">
      ...
      <div className="px-4 mb-6">
        <div className="flex items-center justify-between mb-2">
          <h2 className="text-sm font-bold text-gray-800">Upcoming Consultation</h2>
          <button onClick={() => navigate('/mobile/appointments')} className="text-xs text-primary font-semibold">See all</button>
        </div>
        {upcomingAppointment ? (
          <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">General Physician • Today, 4:00 PM</p>
            </div>
            <button className="ml-auto bg-primary/10 text-primary text-xs font-bold px-3 py-1.5 rounded-lg">Join</button>
          </div>
        ) : (
          <div className="bg-gradient-to-r from-teal-500 to-teal-600 rounded-2xl p-5 text-white shadow-lg relative overflow-hidden">
            <div className="relative z-10 flex flex-col items-start">
              ...
              <p className="text-teal-100 text-xs mb-3 max-w-[70%]">Connect with top doctors for a healthy life.</p>
              <button onClick={() => navigate('/mobile/doctors')} className="bg-white text-teal-600 text-xs font-bold px-4 py-2 rounded-full shadow-sm hover:bg-gray-50 transition-colors">Find a Doctor</button>
            </div>
            <svg className="absolute -bottom-4 -right-4 w-24 h-24 text-white/20 opacity-50 rotate-12" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
          </div>
        )}
      </div>
      ...
      <div className="px-4 mb-8">
        ...
        <div className="grid grid-cols-2 gap-3">
          <button onClick={() => navigate('/medicines')} className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center gap-2 active:scale-95 transition-transform">
            <div className="w-12 h-12 rounded-full bg-orange-100 flex items-center justify-center text-orange-500">
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" /></svg>
            </div>
            <span className="font-bold text-gray-800 text-sm">Order Medicine</span>
          </button>
          <button onClick={() => navigate('/lab-tests')} className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center gap-2 active:scale-95 transition-transform">
            <div className="w-12 h-12 rounded-full bg-purple-100 flex items-center justify-center text-purple-500">
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.384-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z" /></svg>
            </div>
            <span className="font-bold text-gray-800 text-sm">Book Lab Test</span>
          </button>
        </div>
      </div>
      ...
      <section className="mb-6 px-4">
        ...
        <div className="flex gap-3 overflow-x-auto snap-x snap-mandatory pb-4 no-scrollbar">
          {tests.map((t) => (
            <div key={t.id} className="min-w-[200px] snap-start bg-white rounded-2xl border border-gray-100 p-4 shadow-sm flex flex-col justify-between h-32">
              <div>
                <p className="text-sm font-bold text-gray-900 line-clamp-1">{t.name}</p>
                <p className="text-[10px] text-gray-500 mb-3">{t.alias}</p>
              </div>
              <div className="flex items-center justify-between mt-auto">
                <span className="text-sm font-bold text-gray-900">₹{t.price}</span>
                <button onClick={() => navigate('/lab-tests')} className="w-8 h-8 flex items-center justify-center bg-gray-50 rounded-full text-gray-600 hover:bg-primary hover:text-white transition-colors">
      ...
      <section className="mb-8 px-4">
        ...
        <div className="flex gap-3 overflow-x-auto snap-x snap-mandatory pb-4 no-scrollbar">
          {DEFAULT_MEDICINES.slice(0, 5).map((m) => (
            <div key={m.id} className="min-w-[150px] snap-start bg-white rounded-2xl border border-gray-100 p-3 shadow-sm">
              <div className="h-28 bg-gray-50 rounded-xl mb-3 flex items-center justify-center relative overflow-hidden">
                {m.imageUrl ? <img src={m.imageUrl} alt={m.name} className="h-24 w-auto object-contain" /> : <div className="w-12 h-12 bg-gray-200 rounded" />}
                <div className="absolute top-2 right-2 bg-green-100 text-green-700 text-[10px] font-bold px-2 py-0.5 rounded-full">
                  10% OFF
                </div>
              </div>
              <p className="text-xs font-bold text-gray-900 line-clamp-1 mb-1">{m.name}</p>
              <div className="flex items-center justify-between">
                <span className="text-sm font-bold text-gray-900">₹{m.price.toFixed(0)}</span>
                <button onClick={() => navigate('/medicines')} className="p-1.5 bg-primary/10 text-primary rounded-lg">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4"></path></svg>
                </button>
              </div>
            </div>
