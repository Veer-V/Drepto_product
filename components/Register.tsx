...
const Register: React.FC<RegisterProps> = ({ onToggleView }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess(
        'Registration Successful! Use coupon code FIRSTFREE for your first free order!',
      );
    } catch (err: any) {
    ...
    return (
      <div className="text-center py-8">
        ...
        <button
          ...
          className="bg-primary text-white px-6 py-2 rounded-lg font-bold hover:bg-blue-600"
        >
          Go to Dashboard
        </button>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        >
          {Object.values(UserRole).map((r) => (
            <option key={r} value={r}>{`Register as ${r}`}</option>
          ))}
        </select>
        <input
          type="password"
