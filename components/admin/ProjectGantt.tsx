...
const ProjectGantt: React.FC<GanttProps> = ({ project, onUpdate }) => {
  ...
  const [editForm, setEditForm] = useState<{ start: string; end: string }>({
    start: '',
    end: '',
  });
  ...
  const getTimelineHeaders = () => {
    ...
    const minDate = new Date(
      Math.min(
        ...dates.filter((d) => !isNaN(d.getTime())).map((d) => d.getTime()),
      ),
    );
  ...
  if (headers.length === 0) {
    return (
      <div className="p-10 text-center text-gray-500">
        No timeline data available. Ensure tasks have valid dates.
      </div>
    );
  }
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden flex flex-col h-[calc(100vh-200px)]">
      ...
      <div className="p-4 border-b flex justify-between items-center bg-gray-50">
        <div className="flex items-center gap-4">
          ...
          <div className="text-sm text-gray-500 bg-white px-2 py-1 rounded border">
            {headers[0].toLocaleDateString(undefined, {
              month: 'short',
              year: 'numeric',
            })}{' '}
            -{' '}
            {headers[headers.length - 1].toLocaleDateString(undefined, {
              month: 'short',
              year: 'numeric',
            })}
          </div>
      ...
      <div className="overflow-auto flex-1 relative custom-scrollbar">
        ...
        >
          {/* Header Row */}
          <div
            className="sticky top-0 z-10 bg-white/95 backdrop-blur-sm border-b pb-2 font-bold text-gray-600 text-xs uppercase tracking-wider col-span-full grid"
            style={{ gridTemplateColumns: `subgrid` }}
          >
            <div className="sticky left-0 bg-white z-20 pl-2 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">
              Task Name
            </div>
            {headers.map((date, i) => (
              <div
                key={i}
                className="text-center border-l border-gray-100 px-1 truncate text-[10px]"
              >
                {compactMode
                  ? i % 3 === 0
                    ? date.toLocaleDateString(undefined, { month: 'narrow' })
                    : '' // Show fewer labels in compact
          ...
          {project.tasks.map((task) => (
            <div key={task.id} className="contents group hover:bg-gray-50">
              <div className="sticky left-0 bg-white group-hover:bg-gray-50 z-10 py-2 pr-4 border-r border-gray-100 text-sm font-medium text-gray-700 truncate flex items-center gap-2 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">
                ...
                ></span>
                <span title={task.title} className="truncate">
                  {task.title}
                </span>
              </div>
              ...
              <div
                ...
                className={`
                                    ...
                                    ${
                                      task.type === 'phase'
                                        ? 'bg-indigo-500 h-8 rounded-lg'
                                        : task.type === 'sub-phase'
                                          ? 'bg-blue-400 h-6 mt-1'
                                          : task.type === 'milestone'
                                            ? 'bg-amber-500 h-6 mt-1'
              ...
              {headers.map((_, i) => (
                <div
                  key={i}
                  className="border-l border-gray-100 h-full -z-10 row-start-auto"
                  style={{ gridColumn: i + 2, gridRow: 'auto' }}
                ></div>
              ))}
            </div>
          ))}
