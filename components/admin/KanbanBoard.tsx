    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Drag and drop tasks to change status (simulated with clicks)
          </p>
        </div>
        <button
      ...
      {isAdding && (
        <form
          ...
          className="bg-white p-4 rounded-xl border shadow-sm flex gap-3 items-end"
        >
          ...
          <button
            type="submit"
            className="bg-green-600 text-white px-4 py-2 rounded text-sm h-[38px]"
          >
            Add
          </button>
        </form>
      ...
      <div className="grid md:grid-cols-3 gap-6 overflow-x-auto pb-4">
        {columns.map((col) => (
          <div
            key={col.id}
            className={`rounded-xl p-4 ${col.color} min-w-[280px]`}
          >
            ...
            <div className="space-y-3">
              {getTasksByStatus(col.id).map((task) => (
                <div
                  key={task.id}
                  className="bg-white p-3 rounded-lg shadow-sm border border-transparent hover:border-primary/20 group"
                >
