    ...
    <div className="animate-fade-in-up pb-10">
      ...
      <div className="flex items-center mb-8 sticky top-0 bg-gray-50/95 backdrop-blur-sm z-30 py-4 border-b border-gray-100">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-500">
            Choose a plan that fits your health needs
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {plans.map((plan) => (
          <div
            key={plan.name}
            className={`relative p-8 rounded-3xl border flex flex-col transition-all duration-300 hover:shadow-xl ${plan.color} ${plan.recommended ? 'scale-105 shadow-lg z-10' : ''}`}
          >
            {plan.recommended && (
            ...
            <h3
              className={`text-2xl font-bold mb-2 ${plan.name === 'Platinum' ? 'text-white' : 'text-gray-900'}`}
            >
              {plan.name}
            </h3>
            <div
              className={`text-3xl font-bold mb-6 ${plan.name === 'Platinum' ? 'text-blue-300' : 'text-primary'}`}
            >
              {plan.price}
            </div>
            ...
            <ul className="space-y-4 mb-8 flex-1">
              {plan.features.map((feature, i) => (
                <li key={i} className="flex items-start gap-3">
                  <svg
                    className={`w-5 h-5 shrink-0 ${plan.name === 'Platinum' ? 'text-green-400' : 'text-green-500'}`}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                  ...
                  <span
                    className={`text-sm ${plan.name === 'Platinum' ? 'text-gray-300' : 'text-gray-600'}`}
                  >
                    {feature}
                  </span>
                </li>
      ...
      <div className="mt-12 bg-white rounded-2xl p-8 border border-gray-100 text-center">
        ...
        <p className="text-gray-500 mb-6">
          We offer tailored health packages for corporates and large families.
        </p>
