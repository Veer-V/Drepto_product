...
// --- Types ---
interface LabTest {
  id: string; // Changed to string to match Admin ID
  name: string;
  sampleType: string;
  preparation: string;
  price: number;
...
const LabTests: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  useEffect(() => {
    const load = () => {
      const raw = loadLabTests() || [];
      // Map Admin Data to Component State
      const mapped = raw.map((t: any) => ({
        id: t.id,
        name: t.name,
        sampleType: t.sampleType || 'N/A',
        preparation: t.fasting || 'No special preparation',
    ...
    window.addEventListener('storage', onUpdate); // Listen to storage events too
    return () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-12 animate-fade-in-up relative text-left">
      <ComingSoonOverlay
        title="Pathology Lab"
        description="Book home sample collection for blood tests and full body checkups. Opening soon!"
        onBack={onBack}
      />
