...
const DoctorAppointment: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  const filteredDoctors = doctors.filter((d) => {
    ...
    const matchesAvailability =
      availabilityFilter === 'All' ? true : d.available;

    const matchesRating =
      ratingFilter === 'All' || d.rating >= parseFloat(ratingFilter);
      ...
      days.push(
        ...
        >
          ...
          {!isUnavailable && !isSelected && (
            <span
              className={`w-1 h-1 rounded-full absolute bottom-1.5 ${isToday ? 'bg-primary' : 'bg-blue-300'}`}
            ></span>
          )}
        </button>,
      );
      ...
      <div className="bg-white rounded-3xl shadow-lg overflow-hidden border border-gray-100">
        ...
        <div className="p-6 md:p-8">
          <h4 className="font-bold text-gray-800 mb-4 flex items-center gap-2">
            <span className="w-6 h-6 bg-blue-100 text-primary rounded-full flex items-center justify-center text-xs">
              1
            </span>
            Select Date
          </h4>
      ...
      <div className="bg-white rounded-3xl shadow-xl border border-gray-100 w-full overflow-hidden relative">
        ...
        <div className="p-6 bg-gray-50 border-t border-gray-100">
          <button
            ...
            className="w-full bg-primary text-white py-3.5 rounded-xl font-bold hover:bg-blue-600 transition-all shadow-lg shadow-blue-200"
          >
            Return to Dashboard
          </button>
        </div>
      ...
      <div className="mb-8 max-w-4xl mx-auto">
        <div className="relative mb-4">
          ...
          <button
            ...
            className="absolute right-3 top-3 p-1.5 bg-gray-100 hover:bg-gray-200 rounded-lg text-gray-600 md:hidden"
          >
            <svg
              className="w-5 h-5"
              fill="none"
        ...
        <div
          className={`flex flex-col md:flex-row gap-3 justify-center transition-all overflow-hidden ${showFilters ? 'max-h-40 opacity-100 mb-4' : 'max-h-0 opacity-0 md:max-h-full md:opacity-100 md:mb-0'}`}
        >
          <select
          ...
          displayedDoctors.map((doc) => (
            ...
            >
              <div className="p-6 flex items-start gap-4 relative">
                ...
                />
                <div className="absolute top-6 right-6 bg-yellow-50 text-yellow-600 px-2 py-1 rounded-lg text-xs font-bold flex items-center shadow-sm">
                  ⭐ {doc.rating}
                </div>
                <div>
                  ...
                  <p className="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded-md inline-block">
                    Next: {doc.available ? 'Today' : 'Unavailable'}
                  </p>
                </div>
              </div>
      ...
      {filteredDoctors.length > itemsPerPage && (
        <div className="flex justify-center items-center gap-4 mt-10">
          <button
            ...
            className="p-3 rounded-xl border border-gray-300 text-gray-600 disabled:opacity-30 hover:bg-gray-50 transition-colors"
          >
            <svg
              className="w-5 h-5"
              fill="none"
      ...
      {reminderBanner && (
        <div className="bg-gradient-to-r from-blue-500 to-indigo-600 rounded-2xl p-4 text-white shadow-lg mb-6 flex items-center justify-between animate-pulse">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="font-bold text-sm opacity-90 uppercase tracking-wider">
                Up Next
              </p>
              <h4 className="font-bold text-lg">
                Dr. Sarah Smith • Tomorrow, 10:00 AM
              </h4>
            </div>
          </div>
          <button
            onClick={() => setReminderBanner(false)}
      ...
      {!loading && myAppointments.length === 0 ? (
        <div className="text-center py-20 bg-white rounded-3xl border border-dashed border-gray-200">
          ...
          <button
            ...
            className="mt-4 px-6 py-2 bg-primary text-white rounded-lg font-bold text-sm shadow-md hover:bg-blue-600 transition-colors"
          >
            Find a Doctor
          </button>
        </div>
        ...
        myAppointments.map((appt) => (
          ...
          >
            <img
              src={appt.doctor?.image || 'default-avatar.png'}
            ...
            <div className="flex flex-col items-center sm:items-end gap-2 w-full sm:w-auto border-t sm:border-t-0 border-gray-100 pt-4 sm:pt-0">
              <span
                className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide ${appt.status === 'UPCOMING' ? 'bg-green-100 text-green-600' : appt.status === 'COMPLETED' ? 'bg-gray-100 text-gray-500' : 'bg-red-100 text-red-500'}`}
              >
                {appt.status}
              </span>
          ...
          <div className="fixed top-24 right-4 md:right-8 z-50 w-full max-w-sm bg-white rounded-2xl shadow-2xl p-4 border-l-4 border-primary animate-fade-in-up flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-gray-500 mt-1 leading-relaxed">
                You have a consultation with{' '}
                <span className="font-bold text-gray-700">Dr. Sarah Smith</span>{' '}
