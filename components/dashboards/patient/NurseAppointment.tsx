...
const NurseAppointment: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  const getServiceDetails = (id: number | null) =>
    services.find((s) => s.id === id);

  return (
    <div className="min-h-screen animate-fade-in-up pb-10 relative">
      <ComingSoonOverlay
        title="Nurse Services"
        description="Professional nursing care at home is coming soon to your area."
        onBack={onBack}
      />
      {/* Blurred Content */}
              ...
              <div className="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden relative">
                ...
                <div className="bg-emerald-50 p-8 text-center">
                  ...
                  <p className="text-emerald-600/80 mt-1 font-medium">
                    Booking ID: #NUR-{Math.floor(Math.random() * 10000)}
                  </p>
                </div>
                <div className="p-8 space-y-6">
                  ...
                  <button
                    onClick={handleReset}
                    className="w-full py-3.5 bg-emerald-500 text-white font-bold rounded-xl hover:bg-emerald-600 transition-all shadow-lg shadow-emerald-200"
                  >
                    Track Status
                  </button>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {filteredServices.length > 0 ? (
                    filteredServices.map((service) => (
                      ...
                      >
                        <div
                          className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-colors ${selectedService === service.id ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-200' : 'bg-emerald-50 text-emerald-600 group-hover:bg-emerald-100'}`}
                        >
                          {service.icon}
                        </div>
                  ...
                  <form onSubmit={handleBookSubmit} className="space-y-5">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          className="w-full p-3 bg-gray-50 rounded-xl border-transparent focus:border-emerald-500 focus:bg-white focus:ring-0 transition-all text-sm font-medium"
                        />
                      </div>
                      <div>
                        <label className="block text-xs font-bold text-gray-400 uppercase mb-1.5">
                      ...
                      <div className="flex gap-2">
                        {['Any', 'Female', 'Male'].map((g) => (
                          <button
                            ...
                            className={`flex-1 py-2 rounded-lg text-xs font-bold border transition-all ${preferredGender === g ? 'bg-emerald-50 border-emerald-500 text-emerald-700' : 'bg-white border-gray-200 text-gray-500 hover:bg-gray-50'}`}
                          >
                            {g}
                          </button>
                        ))}
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        className="w-full p-3 bg-gray-50 rounded-xl border-transparent focus:border-emerald-500 focus:bg-white focus:ring-0 transition-all text-sm resize-none"
                    ...
                    <div className="pt-4 border-t border-gray-100">
                      ...
                      <button
                        ...
                        className="w-full bg-emerald-500 text-white py-4 rounded-xl font-bold text-lg shadow-xl shadow-emerald-200 hover:bg-emerald-600 hover:shadow-2xl hover:-translate-y-1 transition-all disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
                      >
                        Confirm Request
                      </button>
                    </div>
          ...
          <div className="max-w-3xl mx-auto space-y-6 animate-fade-in-up">
            {appointments.length > 0 ? (
              appointments.map((appt) => (
                <div
                  key={appt.id}
                  className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex flex-col sm:flex-row items-center gap-6 relative overflow-hidden group"
                >
                  <div
                    className={`absolute left-0 top-0 bottom-0 w-1.5 ${appt.status === 'In Progress' ? 'bg-blue-500' : appt.status === 'Scheduled' ? 'bg-yellow-400' : 'bg-emerald-500'}`}
                  ></div>
                  ...
                  <div className="flex-1 text-center sm:text-left">
                    <div className="flex items-center justify-center sm:justify-start gap-2 mb-1">
                      ...
                      <span
                        className={`px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wider ${appt.status === 'In Progress' ? 'bg-blue-50 text-blue-600' : appt.status === 'Scheduled' ? 'bg-yellow-50 text-yellow-600' : 'bg-emerald-50 text-emerald-600'}`}
                      >
                        {appt.status}
                      </span>
                    </div>
                  ...
                  <div className="flex flex-col gap-2 w-full sm:w-auto">
                    {appt.status === 'In Progress' && (
                      <button className="w-full px-5 py-2.5 bg-blue-500 text-white text-sm font-bold rounded-xl hover:bg-blue-600 shadow-lg shadow-blue-200 transition-all">
                        Call Nurse
                      </button>
                    )}
                    {appt.status === 'Scheduled' && (
                      <button className="w-full px-5 py-2.5 bg-white border border-gray-200 text-gray-600 text-sm font-bold rounded-xl hover:bg-gray-50 transition-all">
                        Reschedule
                      </button>
                    )}
                  </div>
              ...
              <div className="text-center py-20 bg-white rounded-3xl border border-dashed border-gray-200">
                ...
                <p className="text-gray-400 mt-1">
                  You haven't booked any nurse visits yet.
                </p>
              </div>
            )}
          </div>
