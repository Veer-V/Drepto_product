...
const Pharmacy: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  const [cart, setCart] = useState<
    { id: number | string; name: string; price: string; image: string }[]
  >([]);
  ...
  return (
    <div className="animate-fade-in-up pb-10 relative text-left">
      <ComingSoonOverlay
        title="Online Pharmacy"
        description="Order medicines online and get them delivered to your doorstep. Launching soon!"
        onBack={onBack}
      />
      {/* Blurred Content */}
      <div className="pointer-events-none filter blur-sm">
        ...
        <div className="bg-gradient-to-r from-purple-600 to-indigo-700 rounded-3xl p-8 text-white mb-10 flex flex-col md:flex-row items-center justify-between shadow-xl relative overflow-hidden">
          ...
          <div className="relative z-10 mb-6 md:mb-0 max-w-md">
            ...
            <p className="text-purple-100 opacity-90">
              Upload your doctor's prescription and let us organize your
              medicine delivery.
            </p>
          </div>
          <div className="relative z-10">
            ...
            {uploadSuccess ? (
              ...
              <button
                ...
                className="bg-white text-purple-700 px-8 py-3.5 rounded-xl font-bold shadow-lg hover:bg-purple-50 hover:scale-105 transition-all flex items-center gap-2 disabled:opacity-80 disabled:cursor-wait min-w-[160px] justify-center"
              >
                {isUploading ? (
        ...
        <div className="mb-8">
          ...
          <div className="flex gap-3 overflow-x-auto pb-4 scrollbar-hide">
            {categories.map((cat) => (
              <button
                key={cat}
                className="flex-shrink-0 bg-white px-5 py-3 rounded-xl border border-gray-200 text-gray-600 font-medium hover:border-purple-500 hover:text-purple-600 hover:shadow-md transition-all whitespace-nowrap"
              >
                {cat}
              </button>
            ))}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {popularItems.length > 0 ? (
              popularItems.map((item) => (
                <div
                  key={item.id}
                  className="bg-white rounded-2xl p-4 shadow-sm hover:shadow-lg transition-all border border-gray-100 group flex flex-col"
                >
                  ...
                  <h4 className="font-bold text-gray-800 mb-1 text-sm line-clamp-2 min-h-[2.5em]">
                    {item.name}
                  </h4>
        ...
        {isCartOpen && (
          <div className="fixed inset-0 z-50 flex justify-end">
            ...
            {/* Cart Panel */}
            <div
              className="relative w-full max-w-md bg-white shadow-2xl flex flex-col h-full animate-slide-in-left"
              style={{
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4">
                {cart.length === 0 ? (
                  ...
                  cart.map((item, index) => (
                    <div
                      key={index}
                      className="flex items-center gap-4 p-3 rounded-2xl border border-gray-100 hover:border-purple-200 transition-all bg-white shadow-sm"
                    >
                      <img
                        src={item.image}
                        alt={item.name}
                ...
                <div className="p-6 border-t border-gray-100 bg-gray-50">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <button
                      ...
                      className="w-full py-3 bg-purple-600 text-white font-bold rounded-2xl hover:bg-purple-700 transition-all shadow-lg shadow-purple-200 active:scale-95"
                    >
                      Proceed to Payment
                    </button>
                  </div>
