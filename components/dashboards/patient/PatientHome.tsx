...
const DashboardCard: React.FC<DashboardCardProps> = ({
  ...
  >
    ...
    <div
      className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-5 ${color} text-white shadow-lg shadow-${color.replace('bg-', '')}/30 group-hover:scale-110 group-hover:rotate-3 transition-all duration-300`}
    >
      {icon}
    </div>

    <h3 className="text-xl font-bold text-gray-800 mb-2 group-hover:translate-x-1 transition-transform duration-300">
      {title}
    </h3>
  ...
  const modules = [
    {
      ...
      description: 'Browse specialists, book slots & manage consultations.',
      icon: <DoctorIcon />,
      color: 'bg-blue-500',
    },
    {
      ...
      description: 'Home care, elderly support & professional nursing.',
      icon: <NurseIcon />,
    ...
    },
    {
      id: 'lab',
      title: 'Lab Tests',
      description: 'Book diagnostics & view reports online.',
      icon: <LabIcon />,
      color: 'bg-indigo-500',
    },
    {
      id: 'products',
      title: 'Drepto Store',
      description: 'Healthcare devices & wellness products.',
      icon: <ProductIcon />,
      color: 'bg-orange-500',
    },

    {
      id: 'ambulance',
      title: 'Ambulance',
      description: 'Emergency 24/7 road & air ambulance.',
      icon: <AmbulanceIcon />,
      color: 'bg-red-500',
    },
    {
      id: 'profile',
      title: 'My Profile',
      description: 'Medical records, history & settings.',
      icon: <SettingsIcon />,
      color: 'bg-gray-700',
    },
  ];
    ...
    <div className="space-y-8 animate-fade-in-up pb-10">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          <div
            key={i}
            className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow flex flex-col justify-between h-full"
          >
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
          {modules.map((m) => (
            ...
            >
              <div>
                <div
                  className={`w-10 h-10 md:w-14 md:h-14 rounded-xl md:rounded-2xl flex items-center justify-center mb-3 md:mb-5 ${m.color} text-white shadow-lg shadow-blue-500/30`}
                >
                  {m.icon}
                </div>
                <h3 className="text-sm md:text-xl font-bold text-gray-800 mb-1 md:mb-2 leading-tight">
                  {m.title}
                </h3>
