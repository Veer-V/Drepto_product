        ...
        <div className="animate-fade-in-up h-[calc(100vh-120px)] flex flex-col relative overflow-hidden rounded-3xl border border-gray-200 shadow-2xl bg-gray-100">
            ...
            <div className="absolute inset-0 pointer-events-none filter blur-sm">
                ...
                <div className="absolute top-0 left-0 right-0 p-6 z-20 flex justify-between items-start pointer-events-none">
                    <button className="bg-white/90 backdrop-blur-md p-2.5 rounded-xl shadow-sm text-gray-700 border border-white/50"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 19l-7-7 7-7"></path></svg></button>
                    <div className="bg-red-600 text-white px-4 py-1.5 rounded-full text-xs font-bold shadow-lg shadow-red-500/40 animate-pulse flex items-center gap-2"><span className="w-2 h-2 bg-white rounded-full"></span>SOS EMERGENCY</div>
                </div>
                        ...
                        <div className="bg-white w-full max-w-2xl rounded-t-[2.5rem] shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.2)] p-6 md:p-8 animate-slide-in-up">
                            ...
                            <div className="space-y-4 mb-6 relative">
                                ...
                                <div className="relative z-10">
                                    ...
                                    <input type="text" placeholder="Pickup Location" value={pickup} readOnly className="w-full pl-10 pr-4 py-3 bg-gray-50 rounded-xl border-transparent focus:border-red-500 focus:bg-white focus:ring-0 outline-none font-medium text-gray-800" />
                                </div>
                                <div className="relative z-10">
                                    <div className="absolute left-3 top-3.5 w-3 h-3 bg-red-500 rounded-sm border-2 border-white shadow-sm"></div>
                                    <input type="text" placeholder="Drop-off (Optional)" value={dropoff} readOnly className="w-full pl-10 pr-4 py-3 bg-gray-50 rounded-xl border-transparent focus:border-red-500 focus:bg-white focus:ring-0 outline-none font-medium text-gray-800" />
                                </div>
                            </div>
                                ...
                                {type === 'Road' ? (
                                    <>
                                        {[{ id: 'basic', l: 'Basic', p: '₹50' }, { id: 'oxygen', l: 'Oxygen', p: '₹80' }, { id: 'icu', l: 'ICU', p: '₹150' }].map(v => (
                                            <div key={v.id} className={`p-3 rounded-2xl border cursor-pointer transition-all text-center ${vehicleType === v.id ? 'border-red-500 bg-red-50 text-red-700' : 'border-gray-100 hover:border-gray-300'}`}>
                            ...
                            <button disabled className="w-full bg-red-600 text-white py-4 rounded-2xl font-bold text-lg shadow-lg shadow-red-500/30 transition-all opacity-50 cursor-not-allowed">Request Now</button>
                        </div>
                    )}
                </div>
            </div>
