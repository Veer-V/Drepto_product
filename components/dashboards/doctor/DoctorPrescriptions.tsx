          ...
          <div className="bg-white p-8 rounded-3xl shadow-xl shadow-purple-50 border border-gray-100 animate-fade-in-up">
            <div className="flex items-center gap-3 mb-6 pb-6 border-b border-gray-100">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">
                  Issue a digital prescription instantly.
                </p>
              </div>
            </div>
            ...
            <form onSubmit={handlePreview} className="space-y-5">
              ...
              <div className="pt-4">
                <button
                  type="submit"
                  className="w-full py-4 bg-purple-600 text-white rounded-2xl font-bold text-lg hover:bg-purple-700 transition-all shadow-lg shadow-purple-200 active:scale-95"
                >
                  Review & Issue
                </button>
              </div>
          ...
          <div className="space-y-4 animate-fade-in-up">
            {prescriptions.length > 0 ? (
              prescriptions.map((rx) => (
                <div
                  key={rx.id}
                  className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-4 hover:shadow-md transition-shadow"
                >
                  <div className="flex items-center gap-4 w-full sm:w-auto">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400 font-medium mt-1 max-w-[200px] truncate">
                        {rx.medications}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 w-full sm:w-auto">
                    <span
                      className={`px-3 py-1 rounded-full text-xs font-bold uppercase ${rx.status === 'Active' ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-500'}`}
                    >
                      {rx.status}
                    </span>
                    <button
                      className="p-2 bg-gray-50 rounded-lg text-gray-500 hover:bg-purple-50 hover:text-purple-600 transition-colors"
                      title="Print"
                    >
                      <svg
                        className="w-5 h-5"
              ...
              <div className="text-center py-24 bg-white rounded-3xl border border-dashed border-gray-200">
                ...
                <p className="text-gray-500 text-sm mb-6">
                  You haven't issued any digital prescriptions yet.
                </p>
                <button
                  onClick={() => setActiveTab('create')}
                  className="text-purple-600 font-bold hover:underline"
                >
                  Create your first Rx
                </button>
              </div>
            )}
          </div>
          ...
          <div className="bg-white w-full max-w-lg rounded-xl shadow-2xl overflow-hidden relative">
            ...
            <div className="p-4 bg-gray-50 border-t border-gray-200 flex gap-3">
              <button
                ...
                className="flex-1 py-3 bg-white border border-gray-300 rounded-xl font-bold text-gray-600 hover:bg-gray-100"
              >
                Edit
              </button>
              <button
                onClick={handleConfirm}
                className="flex-1 py-3 bg-purple-600 text-white rounded-xl font-bold hover:bg-purple-700 shadow-lg"
              >
                Confirm & Issue
              </button>
            </div>
