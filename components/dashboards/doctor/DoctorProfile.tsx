...
const DoctorProfile: React.FC<{ user: User; onBack: () => void }> = ({
  user,
  onBack,
}) => {
  ...
  return (
    <div className="animate-fade-in-up pb-12">
      <button
        onClick={onBack}
        className="mb-8 flex items-center text-gray-600 hover:text-primary transition-colors font-medium group"
      >
      ...
      <div className="bg-white rounded-3xl shadow-lg shadow-gray-100 border border-gray-100 p-8 max-w-3xl mx-auto relative overflow-hidden">
        ...
        <div className="relative flex flex-col items-center -mt-4 mb-8">
          <div className="w-32 h-32 bg-white rounded-full p-1.5 shadow-xl mb-4">
            ...
            <div
              className={`absolute bottom-2 right-2 w-8 h-8 rounded-full border-4 border-white flex items-center justify-center shadow-sm ${status === 'Available' ? 'bg-green-500' : status === 'Busy' ? 'bg-orange-500' : 'bg-gray-400'}`}
            >
              {status === 'Available' && (
                <svg
        ...
        <div className="space-y-8">
          ...
          <div>
            ...
            {/* Live Status Toggle */}
            <div className="mb-6">
              <label className="block text-xs font-bold text-gray-500 mb-3 ml-1">
                Current Status
              </label>
              <div className="flex p-1 bg-gray-100 rounded-2xl">
                {['Available', 'Busy', 'Offline'].map((s) => (
                  ...
                  >
                    ...
                    ></span>
                    <span
                      className={`${status === s ? (s === 'Available' ? 'text-green-600' : s === 'Busy' ? 'text-orange-600' : 'text-gray-600') : ''}`}
                    >
                      {s}
                    </span>
                  </button>
            ...
            >
              <div className="flex items-center gap-4">
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center ${isAcceptingPatients ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-400'}`}
                >
                  <svg
                    className="w-5 h-5"
                    fill="none"
              ...
              <div
                className={`w-14 h-8 rounded-full p-1 transition-colors duration-300 ${isAcceptingPatients ? 'bg-green-500' : 'bg-gray-300'}`}
              >
                <div
                  className={`w-6 h-6 bg-white rounded-full shadow-md transform transition-transform duration-300 ${isAcceptingPatients ? 'translate-x-6' : 'translate-x-0'}`}
                ></div>
              </div>
            </div>
          </div>
