...
const PatientDashboard: React.FC<PatientDashboardProps> = ({ user }) => {
  ...
  const navItems = [
    ...
    { id: 'lab', label: 'Lab Tests', icon: LabIcon },
        ...
        <div className="flex items-center p-2 overflow-x-auto gap-2 no-scrollbar px-4">
          {navItems.map((item) => (
            <button
              key={item.id}
              onClick={() => setCurrentView(item.id)}
              className={`flex flex-col items-center justify-center p-2 rounded-xl transition-all min-w-[70px] shrink-0 ${
                currentView === item.id
            ...
            >
              <item.icon active={currentView === item.id} />
              <span
                className={`text-[10px] font-medium mt-1 truncate w-full text-center ${currentView === item.id ? 'font-bold' : ''}`}
              >
                {item.label}
              </span>
            </button>
