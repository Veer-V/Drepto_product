...
const NurseProfile: React.FC<{ user: User; onBack: () => void }> = ({
  user,
  onBack,
}) => {
        ...
        <div className="space-y-8">
          ...
          <div className="bg-gray-50 p-4 rounded-2xl flex items-center justify-between border border-gray-200">
            <div className="flex items-center gap-4">
              <div
                className={`w-12 h-12 rounded-full flex items-center justify-center ${isOnDuty ? 'bg-emerald-100 text-emerald-600' : 'bg-gray-200 text-gray-500'}`}
              >
                <svg
                  className="w-6 h-6"
                  fill="none"
            ...
            >
              <span
                className={`absolute left-1 top-1 w-7 h-7 bg-white rounded-full shadow-md transform transition-transform duration-300 ${isOnDuty ? 'translate-x-7' : 'translate-x-0'}`}
              ></span>
            </button>
          </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {[
                ...
                <span
                  key={skill}
                  className="px-3 py-1.5 bg-teal-50 text-teal-700 rounded-lg text-xs font-bold border border-teal-100"
                >
                  {skill}
                </span>
              ))}
              {isEditing && (
                <button className="px-3 py-1.5 border border-dashed border-gray-300 text-gray-400 rounded-lg text-xs font-bold hover:text-emerald-500 hover:border-emerald-300 transition-colors">
                  + Add
                </button>
              )}
            </div>
