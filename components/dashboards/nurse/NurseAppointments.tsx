...
const NurseAppointments: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  const renderVisitItem = (visit: Visit) => (
    <div
      key={visit.id}
      className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex flex-col lg:flex-row items-start lg:items-center gap-6 hover:shadow-lg transition-all duration-300 group relative overflow-hidden"
    >
      <div
        className={`absolute left-0 top-0 bottom-0 w-1.5 ${visit.priority === 'High' ? 'bg-red-500' : 'bg-emerald-500'}`}
      ></div>
      ...
      <div className="flex flex-row lg:flex-col gap-2 w-full lg:w-auto mt-2 lg:mt-0 pt-4 lg:pt-0 border-t lg:border-none border-gray-100">
        {visit.status !== 'Completed' ? (
          <>
            <button className="flex-1 lg:flex-none px-5 py-2.5 rounded-xl bg-emerald-500 text-white font-bold text-sm hover:bg-emerald-600 shadow-md shadow-emerald-200 transition-all">
              Start Visit
            </button>
          ...
          <span className="px-4 py-1.5 bg-gray-100 text-gray-500 rounded-full text-xs font-bold uppercase tracking-wider text-center">
            Completed
          </span>
        )}
      </div>
        ...
        {displayedVisits.length > 0 ? (
          ...
          <div className="text-center py-24 bg-white rounded-3xl border border-dashed border-gray-200 animate-fade-in-up">
            ...
            <p className="text-gray-500 text-sm max-w-xs mx-auto">
              You have no {activeTab} visits scheduled matching your search.
            </p>
          </div>
        )}
      </div>
