  ...
  return (
    <div className="animate-fade-in-up pb-12 relative">
      <div className="flex items-center justify-between mb-8">
        <button
          onClick={onBack}
          className="flex items-center text-gray-600 hover:text-primary transition-colors font-medium group"
        >
        ...
        <div className="space-y-3">
          {filteredTasks.length > 0 ? (
            filteredTasks.map((task) => (
              ...
              >
                <div
                  className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors ${task.status === 'Completed' ? 'bg-emerald-500 border-emerald-500' : 'border-gray-300 group-hover:border-emerald-400'}`}
                >
                ...
                <div className="flex-1">
                  <p
                    className={`font-bold text-gray-800 ${task.status === 'Completed' ? 'line-through text-gray-400' : ''}`}
                  >
                    {task.description}
                  </p>
                  <div className="flex items-center gap-3 mt-1 text-xs">
                    <span className="text-gray-500 font-medium bg-gray-50 px-2 py-0.5 rounded">
                ...
                <span
                  className={`px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase border ${getPriorityColor(task.priority)}`}
                >
                  {task.priority}
                </span>
              </div>
            ...
            <div className="text-center py-16 bg-white rounded-3xl border border-dashed border-gray-200">
              ...
              <p className="text-gray-400 text-sm mb-4">
                Your list is currently empty for this filter.
              </p>
              <button
                onClick={() => setIsAddTaskOpen(true)}
            ...
            <form onSubmit={handleAddTask} className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full p-3 bg-gray-50 rounded-xl border-transparent focus:border-orange-500 focus:bg-white focus:ring-0 transition-all"
              ...
              <button
                type="submit"
                className="w-full bg-orange-500 text-white py-3 rounded-xl font-bold shadow-lg shadow-orange-200 hover:bg-orange-600 transition-all mt-2"
              >
                Add Task
              </button>
            </form>
